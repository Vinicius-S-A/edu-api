...
exports.findById = async (req, res) => {
  try {
    ...
    const course = await knex.select('*')
    .from('courses')
    .where({id})
    .first()

    if (!course){
      return res.status(404).send(`curso com o id ${id} não foi encontrado`)
    }
  ...
  try {
    ...
    if (!course) {
      return res.status(404).send({
        status: `Nenhum curso ${id} foi encontrado`
      });
    }
